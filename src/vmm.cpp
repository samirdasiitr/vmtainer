...
bool Vmm::query_msr_list() {
  ...
  // Description: Queries the list of Model-Specific Registers (MSRs) supported by host KVM.
  // Arch Support: x86 only.
  // Important Notes: System ioctl. Caller sets 'nmsrs' to buffer capacity; KVM overwrites it
  // with the total number of available MSRs and fills indices. Returns -E2BIG if buffer too small.
...
bool Vmm::init(bool zero_ram) {
  ...
  // Description: Queries the KVM kernel module API version.
  // Arch Support: All KVM architectures (x86, arm64, riscv, s390, etc.).
  // Important Notes: System ioctl. Must return exactly 12; any other value indicates an
  // incompatible KVM ABI version. Takes no argument (0).
  ...
  // ioctl: KVM_CREATE_VM
  // Description: Creates a new virtual machine instance and returns a VM file descriptor.
  // Arch Support: All KVM architectures (x86, arm64, riscv, s390, etc.).
  // Important Notes: System ioctl. Argument specifies machine type (0 = default/legacy).
  // The resulting VM fd manages memory slots, in-kernel interrupt chips, and vCPUs.
  ...
  // Description: Creates an in-kernel virtual interrupt controller.
  // Arch Support: x86 (PIC + IOAPIC + Local APIC), ARM/ARM64 (vGICv2), s390.
  // Important Notes: VM ioctl. On x86, initializes master PIC, slave PIC, and IOAPIC in
  // kernel space, and prepares Local APIC emulation for subsequently created vCPUs. Must be
  // invoked before vCPUs are created.
  ...
  // Description: Creates an in-kernel 8254 Programmable Interval Timer (PIT).
  // Arch Support: x86 only.
  // Important Notes: VM ioctl. Supersedes KVM_CREATE_PIT. KVM_PIT_SPEAKER_DUMMY avoids
  // emulating the PC speaker port 0x61. Requires in-kernel IRQCHIP.
  ...
  ram_ = mmap(nullptr, ram_bytes_, PROT_READ | PROT_WRITE, MAP_SHARED,
              ram_memfd_, 0);
  if (ram_ == MAP_FAILED) {
    perror("mmap ram");
    return false;
  ...
  DBG("init: ram=%p memfd=%d size=%zuMB hugetlb=%d", ram_, ram_memfd_,
      ram_bytes_ >> 20, use_hugetlb_);
  ...
  if (!set_memslot(0, 0, ram_, ram_bytes_))
    return false;

  // ioctl: KVM_CREATE_VCPU
  // Description: Instantiates a virtual CPU (vCPU) thread within the VM and returns its fd.
  // Arch Support: All KVM architectures (x86, arm64, riscv, s390, etc.).
  // Important Notes: VM ioctl. Takes integer vCPU ID (0 for single-vCPU micro-VM). Each vCPU
  // runs on its own host thread. Maximum vCPU count can be queried via KVM_CAP_MAX_VCPUS.
  ...
  // Description: Returns the size in bytes of the vCPU execution control block ('struct kvm_run').
  // Arch Support: All KVM architectures (x86, arm64, riscv, s390, etc.).
  // Important Notes: System ioctl. The returned byte size is mmap'd against each vcpu_fd to
  // inspect VM exit reasons, exchange I/O port data, and coordinate run states.
  run_mmap_sz_ = ioctl(kvm_fd_, KVM_GET_VCPU_MMAP_SIZE, 0);
  kvm_run_ = (struct kvm_run *)mmap(
      nullptr, run_mmap_sz_, PROT_READ | PROT_WRITE, MAP_SHARED, vcpu_fd_, 0);
  ...
  // Description: Queries CPUID leaves and feature bits supported by KVM on the host CPU.
  // Arch Support: x86 only.
  // Important Notes: System ioctl. Returns hardware-supported and hypervisor-emulated CPUID
  // functions. The result is filtered or passed directly to the vCPU via KVM_SET_CPUID2.
...
bool Vmm::set_memslot(uint32_t slot, uint64_t gpa, void *hva, size_t len) {
  ...
  // Description: Maps a host virtual address (HVA) buffer into a guest physical address (GPA) slot.
  // Arch Support: All KVM architectures (x86, arm64, riscv, s390, etc.).
  // Important Notes: VM ioctl. Maps guest physical memory to userspace memory (memfd/mmap).
  // GPA and size must be page-aligned (4KB or huge pages). Setting memory_size to 0 removes the slot.
...
void Vmm::install_irq(uint16_t vec, unsigned long addr, const void *code,
                      size_t size) {
...
bool Vmm::load_bzimage(const char *path) {
  ...
  uint16_t ss =
      bp.hdr.setup_sects ? bp.hdr.setup_sects : BZ_DEFAULT_SETUP_SECTS;
...
bool Vmm::setup_cpu() {
  ...
  // ioctl: KVM_GET_SREGS
  // Description: Reads x86 special registers (CS, DS, ES, FS, GS, SS, GDT, IDT, CR0-CR4, EFER).
  // Arch Support: x86 only.
  // Important Notes: vCPU ioctl. Used during setup to read default register states before
  // reconfiguring segment registers and control registers for real-mode execution.
  ...
  // ioctl: KVM_SET_SREGS
  // Description: Sets x86 special registers (segment selectors/bases, descriptor tables, CR0-CR4, EFER).
  // Arch Support: x86 only.
  // Important Notes: vCPU ioctl. Configures processor operating mode (here, 16-bit real mode matching
  // the Linux x86 boot protocol). Inconsistent or illegal CR0/CR4/EFER bits return -EINVAL.
  ...
  regs.rflags = 0x2;
  // ioctl: KVM_SET_REGS
  // Description: Sets x86 general-purpose registers (RAX..RDX, RSI, RDI, RSP, RBP, RIP, RFLAGS).
  // Arch Support: x86 only.
  // Important Notes: vCPU ioctl. Initializes entry instruction pointer (RIP=0x0200) and stack pointer
  // (RSP=0x8000) for starting Linux kernel real-mode setup code.
  ...
  // Description: Configures the CPUID instruction response model visible to the vCPU.
  // Arch Support: x86 only.
  // Important Notes: vCPU ioctl. Must be invoked before KVM_RUN. Defines features, vendor ID,
  // hypervisor signatures, and cache topologies reported to the guest kernel.
...
void Vmm::serial_in(uint16_t port, uint8_t *data) {
  ...
  switch (port) {
  case 0x3f8:
    *data = dlab ? uart_dll_ : 0;
    break;
  case 0x3f9:
    *data = dlab ? uart_dlh_ : uart_ier_;
    break;
  case 0x3fa:
    *data = (uart_ier_ & 2) ? 0x02 : 0x01;
    break;
  case 0x3fb:
    *data = uart_lcr_;
    break;
  ...
  case 0x3f8:
    ...
    if (data == '\n') {
      ...
      if (timing_entrypoint_ && !entrypoint_measured_) {
        if (serial_line_buf_.find("VMTAINER: running entrypoint") !=
            std::string::npos) {
    ...
    }
    {
      ...
      irq.irq = 4;
      // ioctl: KVM_IRQ_LINE
      // Description: Asserts or de-asserts a line on the in-kernel interrupt controller.
      // Arch Support: x86 (PIC/IOAPIC pins), ARM/ARM64 (vGIC).
      // Important Notes: VM ioctl. Pulsing level=1 then level=0 injects an edge-triggered
      // interrupt (here, IRQ 4 signals COM1 UART 16550 transmit buffer empty).
      irq.level = 1;
...
int Vmm::run() {
  ...
  for (;;) {
    ...
    // ioctl: KVM_RUN
    // Description: Enters guest execution (hardware virtualization mode) until a VM-exit occurs.
    // Arch Support: All KVM architectures (x86, arm64, riscv, s390, etc.).
    // Important Notes: vCPU ioctl. Synchronously executes guest code on the host thread until
    // an exit event (PIO, MMIO, HLT, shutdown, error) populates 'kvm_run->exit_reason'.
    // Returns -1 with errno=EINTR when interrupted by host signals.
    if (ioctl(vcpu_fd_, KVM_RUN, 0) < 0) {
      if (errno == EINTR) {
        struct kvm_regs regs;
        // ioctl: KVM_GET_REGS
        // Description: Retrieves current general-purpose registers to inspect flags on signal interruption.
        // Arch Support: x86 only.
        // Important Notes: vCPU ioctl. Checks if the RFLAGS interrupt flag (IF = 0x200) is set.
        ioctl(vcpu_fd_, KVM_GET_REGS, &regs);
        if (!(regs.rflags & 0x200)) {
          struct kvm_mp_state mp = {};
          // ioctl: KVM_GET_MP_STATE
          // Description: Queries multi-processor execution state (runnable, halted, stopped, etc.).
          // Arch Support: x86, arm64, s390.
          // Important Notes: vCPU ioctl. Determines whether the interrupted vCPU was halted (KVM_MP_STATE_HALTED).
    ...
    case KVM_EXIT_HLT: {
      ...
      // ioctl: KVM_GET_REGS
      // Description: Inspects vCPU registers upon HLT instruction VM-exit.
      // Arch Support: x86 only.
      // Important Notes: vCPU ioctl. Verifies if interrupts are disabled (IF=0x200 unset);
      // a halted vCPU with interrupts disabled indicates guest shutdown / poweroff.
    ...
    case KVM_EXIT_FAIL_ENTRY:
      fprintf(stderr, "FAIL_ENTRY: 0x%llx\n",
              (unsigned long long)
                  kvm_run_->fail_entry.hardware_entry_failure_reason);
      return -1;
...
bool Vmm::save_snapshot(Snapshot &snap) {
  ...
  // vCPU registers
  // ioctl: KVM_GET_REGS
  // Description: Reads all general-purpose registers into the snapshot header.
  // Arch Support: x86 only.
  // Important Notes: vCPU ioctl. Captures RIP, RSP, RFLAGS, RAX..R15 for deterministic restore.
  if (ioctl(vcpu_fd_, KVM_GET_REGS, &h.regs) < 0) {
    perror("get regs");
    return false;
  }
  // ioctl: KVM_GET_SREGS
  // Description: Reads special registers (segment registers, GDT, IDT, CR0-CR4, EFER).
  // Arch Support: x86 only.
  // Important Notes: vCPU ioctl. Captures CPU execution mode, paging state, and segment bases.
  if (ioctl(vcpu_fd_, KVM_GET_SREGS, &h.sregs) < 0) {
    perror("get sregs");
    return false;
  }
  // ioctl: KVM_GET_LAPIC
  // Description: Reads in-kernel Local APIC state page (APIC registers, timers, vectors).
  // Arch Support: x86 only.
  // Important Notes: vCPU ioctl. Requires in-kernel IRQCHIP. Saves full 4KB APIC register page.
  if (ioctl(vcpu_fd_, KVM_GET_LAPIC, &h.lapic) < 0) {
    perror("get lapic");
    return false;
  }
  // ioctl: KVM_GET_VCPU_EVENTS
  // Description: Queries pending exception and interrupt event injections (NMI, SMI, SIPI).
  // Arch Support: x86, arm64.
  // Important Notes: vCPU ioctl. Preserves any in-flight unserviced exception across snapshot.
  if (ioctl(vcpu_fd_, KVM_GET_VCPU_EVENTS, &h.events) < 0) {
    perror("get events");
    return false;
  }
  // ioctl: KVM_GET_XCRS
  // Description: Reads extended control registers (XCR0) managing AVX/SSE processor state.
  // Arch Support: x86 only.
  // Important Notes: vCPU ioctl. Defines active feature masks needed to interpret saved XSAVE data.
  if (ioctl(vcpu_fd_, KVM_GET_XCRS, &h.xcrs) < 0) {
    perror("get xcrs");
    return false;
  }
  // ioctl: KVM_GET_MP_STATE
  // Description: Queries multi-processor execution state (runnable, halted, stopped).
  // Arch Support: x86, arm64, s390.
  // Important Notes: vCPU ioctl. Preserves execution state across snapshot.
  if (ioctl(vcpu_fd_, KVM_GET_MP_STATE, &h.mp_state) < 0) {
    perror("get mp_state");
    return false;
  }
  // ioctl: KVM_GET_DEBUGREGS
  // Description: Reads hardware debug registers (DR0-DR7).
  // Arch Support: x86 only.
  // Important Notes: vCPU ioctl. Saves active hardware breakpoints and watchpoints.
  if (ioctl(vcpu_fd_, KVM_GET_DEBUGREGS, &h.debugregs) < 0) {
    perror("get debugregs");
    return false;
  }

  // VM-global state
  // ioctl: KVM_GET_CLOCK
  // Description: Reads the current VM system clock (kvmclock monotonic nanoseconds base).
  // Arch Support: x86 only.
  // Important Notes: VM ioctl. Preserves time continuity across snapshot save/restore.
  if (ioctl(vm_fd_, KVM_GET_CLOCK, &h.clock) < 0) {
    perror("get clock");
    return false;
  }
  // ioctl: KVM_GET_PIT2
  // Description: Reads internal state of the in-kernel 8254 PIT (channels, counter, load time).
  // Arch Support: x86 only.
  // Important Notes: VM ioctl. Saves timer channels and periodic tick schedules.
  if (ioctl(vm_fd_, KVM_GET_PIT2, &h.pit) < 0) {
    perror("get pit");
    return false;
  }

  // ioctl: KVM_GET_IRQCHIP
  // Description: Reads in-kernel interrupt controller state (master PIC, slave PIC, IOAPIC).
  // Arch Support: x86 only.
  // Important Notes: VM ioctl. Caller sets chip_id (KVM_IRQCHIP_PIC_MASTER, PIC_SLAVE, IOAPIC).
  h.pic_master.chip_id = KVM_IRQCHIP_PIC_MASTER;
  h.pic_slave.chip_id = KVM_IRQCHIP_PIC_SLAVE;
  h.ioapic.chip_id = KVM_IRQCHIP_IOAPIC;
  if (ioctl(vm_fd_, KVM_GET_IRQCHIP, &h.pic_master) < 0) {
    perror("get pic0");
    return false;
  }
  if (ioctl(vm_fd_, KVM_GET_IRQCHIP, &h.pic_slave) < 0) {
    perror("get pic1");
    return false;
  }
  if (ioctl(vm_fd_, KVM_GET_IRQCHIP, &h.ioapic) < 0) {
    perror("get ioapic");
    return false;
  }

  // ioctl: KVM_GET_TSC_KHZ
  // Description: Reads the guest vCPU Time Stamp Counter (TSC) frequency in KHz.
  // Arch Support: x86 only.
  // Important Notes: vCPU ioctl. Recorded into snapshot header to lock frequency on restore.
  ...
  // ioctl: KVM_GET_XSAVE2 / KVM_GET_XSAVE
  // Description: Reads extended processor registers (FPU, MMX, SSE, AVX, AVX-512, PKRU).
  // Arch Support: x86 only.
  // Important Notes: vCPU ioctl. KVM_GET_XSAVE2 adapts to CPU feature size; falls back
  // to KVM_GET_XSAVE if unsupported on older host kernels.
  ...
  // ioctl: KVM_GET_MSRS
  // Description: Reads model-specific registers requested in the msrs->entries index array.
  // Arch Support: x86 only.
  // Important Notes: vCPU ioctl. Saves MSR values (SYSCALL entrypoints, FS/GS base, TSC, etc.).
  ...
  if (snap.ram == MAP_FAILED) {
    perror("mmap snap ram");
    return false;
  }
...
bool Vmm::init_uffd() {
  ...
  api.features = 0;
  // ioctl: UFFDIO_API
  // Description: Initiates userfaultfd API handshake and negotiates feature flags with the kernel.
  // Arch Support: Linux generic (all architectures supporting CONFIG_USERFAULTFD).
  // Important Notes: userfaultfd ioctl. Must be called immediately after userfaultfd syscall.
  ...
  // Description: Registers a virtual memory address range to intercept page faults via userfaultfd.
  // Arch Support: Linux generic (all architectures supporting CONFIG_USERFAULTFD).
  // Important Notes: userfaultfd ioctl. UFFDIO_REGISTER_MODE_MISSING traps reads/writes to unpopulated pages.
  ...
  DBG("init_uffd: registered userfaultfd on [%p, %p) sz=%zuMB", ram_,
      (char *)ram_ + ram_bytes_, ram_bytes_ >> 20);
  return true;
}

void Vmm::start_uffd_thread() {
  if (uffd_ < 0 || uffd_running_.load())
    return;
...
void Vmm::uffd_worker() {
  ...
  while (uffd_running_.load(std::memory_order_relaxed)) {
    ...
    if (msg.event == UFFD_EVENT_PAGEFAULT) {
      ...
      if (fault_addr < (uint64_t)ram_ ||
          fault_addr >= (uint64_t)ram_ + ram_bytes_) {
      ...
      if (is_dirty && snap_ram_) {
        ...
        copy.copy = 0;
        // ioctl: UFFDIO_COPY
        // Description: Resolves a missing page fault by copying 4KB from snapshot memory and resuming the vCPU.
        // Arch Support: Linux generic (all architectures supporting CONFIG_USERFAULTFD).
        // Important Notes: userfaultfd ioctl. Atomic copy and mapping of a dirty page into guest RAM.
        ...
        // Description: Resolves a missing page fault on clean memory by mapping a kernel zero-page directly.
        // Arch Support: Linux generic (all architectures supporting CONFIG_USERFAULTFD).
        // Important Notes: userfaultfd ioctl. Maps zero-filled pages without userspace memory copying.
  ...
  uffd_fault_count_.store(total_faults, std::memory_order_relaxed);
  DBG("uffd_worker: finished, handled %zu page faults (%zu KB)", total_faults,
      total_faults * 4);
}
...
bool Vmm::restore_snapshot(const Snapshot &snap) {
  ...
  DBG("restore_snapshot: magic=0x%lx ver=%u ram_mb=%u bitmap=%u bytes", h.magic,
      h.version, h.ram_mb, h.bitmap_bytes);
  if (h.magic != SNAP_MAGIC || h.version != SNAP_VERSION ||
      h.ram_mb != ram_mb_) {
  ...
  if (use_uffd_ && !midway_uffd_ && uffd_ >= 0) {
    ...
    auto prefault_gpa = [&](uint64_t gpa) {
      ...
      if (is_dirty && src) {
        ...
        copy.len = PAGE;
        // ioctl: UFFDIO_COPY
        // Description: Pre-populates virtqueue ring pages from snapshot memory prior to vCPU resume.
        // Arch Support: Linux generic (all architectures supporting CONFIG_USERFAULTFD).
        // Important Notes: userfaultfd ioctl. Prevents vhost-user deadlocks on descriptor rings.
        ...
        zero.range.len = PAGE;
        // ioctl: UFFDIO_ZEROPAGE
        // Description: Pre-maps clean virtqueue ring pages with kernel zero-fill.
        // Arch Support: Linux generic (all architectures supporting CONFIG_USERFAULTFD).
        // Important Notes: userfaultfd ioctl. Maps unwritten ring pages before execution begins.
    ...
    if (!snap.dirty_bitmap.empty()) {
      ...
      auto dynamic_copy_worker = [](void *arg) -> void * {
        ...
        while (true) {
          ...
          while (pg < pg_end) {
            ...
            local_dirty += run_len;
            memcpy(c->dst + run_start * 4096, c->src + run_start * 4096,
                   run_len * 4096);
          }
        }
      ...
      DBG("restore: bitmap copy %zu/%zu pages (%zuKB) [%d threads, dynamic "
          "coalesced]",
      ...
      for (size_t off = 0; off < ram_bytes_; off += PAGE) {
        ...
        for (size_t w = 0; w < PAGE / sizeof(uint64_t); w += 8)
          acc |= p[w] | p[w + 1] | p[w + 2] | p[w + 3] | p[w + 4] | p[w + 5] |
                 p[w + 6] | p[w + 7];
        if (acc)
          memcpy(dst + off, src + off, PAGE);
      }
    }
    ...
    if (midway_uffd_) {
      if (!init_uffd()) {
        ...
        start_uffd_thread();
        DBG("restore: midway uffd active (snapshot copied upfront, remainder "
            "via uffd)");
      }
    }
  }
  ...
  if (h.tsc_khz > 0) {
    ...
    // Description: Sets the guest vCPU Time Stamp Counter (TSC) frequency in KHz.
    // Arch Support: x86 only.
    // Important Notes: vCPU ioctl. Pins TSC frequency to match snapshot state, preventing clock drift.
    ioctl(vcpu_fd_, KVM_SET_TSC_KHZ, (unsigned long)h.tsc_khz);
  }

  // ioctl: KVM_SET_CPUID2
  // Description: Restores the CPUID leaves and feature configuration from the snapshot.
  // Arch Support: x86 only.
  // Important Notes: vCPU ioctl. Must be configured before restoring registers and running vCPU.
  if (ioctl(vcpu_fd_, KVM_SET_CPUID2, cpuid_) < 0) {
    perror("set cpuid");
    return false;
  }
  // ioctl: KVM_SET_MP_STATE
  // Description: Restores multi-processor execution status (runnable, halted, stopped).
  // Arch Support: x86, arm64, s390.
  // Important Notes: vCPU ioctl. Restores vCPU to runnable execution state.
  if (ioctl(vcpu_fd_, KVM_SET_MP_STATE, &h.mp_state) < 0) {
    perror("set mp_state");
    return false;
  }
  // ioctl: KVM_SET_SREGS
  // Description: Restores special registers (CR0-CR4, EFER, segment selectors/bases, GDT, IDT).
  // Arch Support: x86 only.
  // Important Notes: vCPU ioctl. Restores 64-bit long mode, paging tables (CR3), and segment descriptors.
  if (ioctl(vcpu_fd_, KVM_SET_SREGS, &h.sregs) < 0) {
    perror("set sregs");
    return false;
  }
  // ioctl: KVM_SET_REGS
  // Description: Restores general-purpose registers (RIP, RSP, RAX..R15, RFLAGS).
  // Arch Support: x86 only.
  // Important Notes: vCPU ioctl. Restores instruction pointer to the post-snapshot MMIO instruction.
  if (ioctl(vcpu_fd_, KVM_SET_REGS, &h.regs) < 0) {
    perror("set regs");
    return false;
  }
  // ioctl: KVM_SET_XCRS
  // Description: Restores extended control registers (XCR0) managing AVX/SSE processor state.
  // Arch Support: x86 only.
  // Important Notes: vCPU ioctl. Must precede KVM_SET_XSAVE so CPU extended feature masks match.
  if (ioctl(vcpu_fd_, KVM_SET_XCRS, &h.xcrs) < 0) {
    perror("set xcrs");
    return false;
  }
  // ioctl: KVM_SET_XSAVE
  // Description: Restores extended processor register states (FPU/MMX/SSE/AVX) from snapshot buffer.
  // Arch Support: x86 only.
  // Important Notes: vCPU ioctl. Requires XCR0 feature bits to match the saved register masks.
  if (ioctl(vcpu_fd_, KVM_SET_XSAVE, (void *)snap.xsave) < 0) {
    perror("set xsave");
    return false;
  }
  // ioctl: KVM_SET_DEBUGREGS
  // Description: Restores hardware debug registers (DR0-DR7).
  // Arch Support: x86 only.
  // Important Notes: vCPU ioctl. Restores hardware breakpoint and watchpoint settings.
  if (ioctl(vcpu_fd_, KVM_SET_DEBUGREGS, &h.debugregs) < 0) {
    perror("set debugregs");
    return false;
  }
  // ioctl: KVM_SET_LAPIC
  // Description: Restores in-kernel Local APIC state page (APIC registers, timers, vectors).
  // Arch Support: x86 only.
  // Important Notes: vCPU ioctl. Reinstates interrupt priority, mask registers, and timer ticks.
  if (ioctl(vcpu_fd_, KVM_SET_LAPIC, &h.lapic) < 0) {
    perror("set lapic");
    return false;
  }
  // ioctl: KVM_SET_VCPU_EVENTS
  // Description: Injects pending exception or interrupt events preserved during snapshotting.
  // Arch Support: x86, arm64.
  // Important Notes: vCPU ioctl. Restores pending interrupts/NMIs without dropping signals.
  if (ioctl(vcpu_fd_, KVM_SET_VCPU_EVENTS, &h.events) < 0) {
    perror("set events");
    return false;
  }
  ...
  // ioctl: KVM_SET_MSRS
  // Description: Writes model-specific registers from the snapshot list back to the vCPU.
  // Arch Support: x86 only.
  // Important Notes: vCPU ioctl. Restores kernel system call entrypoints, segment bases, and MTRRs.
  ioctl(vcpu_fd_, KVM_SET_MSRS, msrs);
  free(msrs);

  // IRQ chips
  // ioctl: KVM_SET_IRQCHIP
  // Description: Restores internal states of master PIC, slave PIC, and IOAPIC from snapshot.
  // Arch Support: x86 only.
  // Important Notes: VM ioctl. Restores interrupt redirection tables, masks, and request registers.
  if (ioctl(vm_fd_, KVM_SET_IRQCHIP, &h.pic_master) < 0) {
    perror("set pic0");
    return false;
  }
  if (ioctl(vm_fd_, KVM_SET_IRQCHIP, &h.pic_slave) < 0) {
    perror("set pic1");
    return false;
  }
  if (ioctl(vm_fd_, KVM_SET_IRQCHIP, &h.ioapic) < 0) {
    perror("set ioapic");
    return false;
  }
  // ioctl: KVM_SET_CLOCK
  // Description: Restores the guest monotonic system clock base to eliminate time jumps.
  // Arch Support: x86 only.
  // Important Notes: VM ioctl. Sets kvmclock nanoseconds base to the saved snapshot timestamp.
  if (ioctl(vm_fd_, KVM_SET_CLOCK, &h.clock) < 0) {
    perror("set clock");
    return false;
  }
  // ioctl: KVM_SET_PIT2
  // Description: Restores the in-kernel 8254 Programmable Interval Timer (PIT) state.
  // Arch Support: x86 only.
  // Important Notes: VM ioctl. Restores timer counter reload values, channels, and gating mode.
  if (ioctl(vm_fd_, KVM_SET_PIT2, &h.pit) < 0) {
    perror("set pit");
    return false;
  }
  ...
  vdrv_features_ =
      (uint64_t)h.vdrv_features_lo | ((uint64_t)h.vdrv_features_hi << 32);
  for (uint32_t i = 0; i < NUM_QUEUES && i < h.num_vqs; i++) {
  ...
  net_drv_features_ =
      (uint64_t)h.net_drv_features_lo | ((uint64_t)h.net_drv_features_hi << 32);
  for (uint32_t i = 0; i < NET_NUM_QUEUES && i < h.net_num_vqs; i++) {
  ...
  // Ensure serial UART (16550) is ready to handle guest userspace console
  // writes
  uart_ier_ = 0x0f;
  uart_lsr_ = 0x60;
  uart_lcr_ = 0x03;
  ...
  DBG("restore_snapshot breakdown: ram_copy=%.2fms kvm_state=%.2fms "
      "total=%.2fms",
      us_diff(ts0, ts1) / 1000.0, us_diff(ts1, ts2) / 1000.0,
...
bool Vmm::save_snapshot_file(const char *path) {
  ...
  printf("[VMM] snapshot: %zu/%zu pages dirty (%.0f%%)\n", dirty_count,
         total_pages, dirty_count * 100.0 / total_pages);
  ...
  ssize_t nw;
  nw = write(fd, &snap.hdr, sizeof(snap.hdr));
  (void)nw;
  nw = write(fd, snap.xsave, snap.hdr.xsave_size);
  (void)nw;
  nw = write(fd, snap.cpuid.data(),
             snap.cpuid.size() * sizeof(kvm_cpuid_entry2));
  (void)nw;
  nw = write(fd, snap.msrs.data(), snap.msrs.size() * sizeof(kvm_msr_entry));
  (void)nw;
  nw = write(fd, bitmap.data(), bm_bytes);
  (void)nw;
...
bool Vmm::restore_snapshot_file(const char *path) {
  ...
  p += sizeof(snap.hdr);
  remain -= sizeof(snap.hdr);

  if (remain < snap.hdr.xsave_size) {
    munmap(map, st.st_size);
    close(fd);
    return false;
  }
  memcpy(snap.xsave, p, snap.hdr.xsave_size);
  p += snap.hdr.xsave_size;
  remain -= snap.hdr.xsave_size;
  ...
  if (snap.hdr.bitmap_bytes > 0) {
    if (remain < snap.hdr.bitmap_bytes) {
      munmap(map, st.st_size);
      close(fd);
      return false;
    }
    ...
    p += snap.hdr.bitmap_bytes;
    remain -= snap.hdr.bitmap_bytes;
  }

  snap.ram_size = snap.hdr.ram_mb * 1024ULL * 1024;
  if (remain < snap.ram_size) {
    munmap(map, st.st_size);
    close(fd);
    return false;
  }
  ...
  DBG("restore_file breakdown: mmap=%.2fms parse=%.2fms restore=%.2fms "
      "total=%.2fms",
      us_diff(tf0, tf1) / 1000.0, us_diff(tf1, tf2) / 1000.0,
      us_diff(tf2, tf3) / 1000.0, us_diff(tf0, tf3) / 1000.0);
...
void Vmm::cleanup() {
  ...
  if (ram_ && ram_ != MAP_FAILED)
    munmap(ram_, ram_bytes_);
  if (ram_memfd_ >= 0)
    close(ram_memfd_);
  if (vu_sock_ >= 0)
    close(vu_sock_);
  if (vu_backend_sock_ >= 0)
    close(vu_backend_sock_);
  for (auto &q : vqs_) {
    if (q.kick_fd >= 0)
      close(q.kick_fd);
    if (q.call_fd >= 0)
...
static void usage(const char *prog) {
  fprintf(
      stderr,
      "Usage:\n"
      "  %s boot   <bzImage> <initrd> --share <dir> [--snapshot <path>] [--ram "
      "<mb>]\n"
...
static bool parse_mac(const char *s, uint8_t mac[6]) {
  unsigned int m[6];
  if (sscanf(s, "%x:%x:%x:%x:%x:%x", &m[0], &m[1], &m[2], &m[3], &m[4],
             &m[5]) != 6)
    return false;
  for (int i = 0; i < 6; i++)
    mac[i] = (uint8_t)m[i];
...
static void write_config_to_share(const char *share_dir, const VmConfig &cfg) {
  ...
  if (cfg.entrypoint[0]) {
    ...
    if (fd >= 0) {
      ssize_t nw;
      nw = ::write(fd, cfg.entrypoint, strlen(cfg.entrypoint));
      (void)nw;
      nw = ::write(fd, "\n", 1);
      (void)nw;
      close(fd);
  ...
  if (cfg.has_net)
    fprintf(f, "NET_MAC=%02x:%02x:%02x:%02x:%02x:%02x\n", cfg.mac[0],
            cfg.mac[1], cfg.mac[2], cfg.mac[3], cfg.mac[4], cfg.mac[5]);
...
int main(int argc, char **argv) {
  ...
  if (strcmp(cmd, "boot") == 0) {
    ...
    Vmm vmm(ram_mb);
    // Optimization: skip manual zeroing -- memfd memory is kernel-zeroed on
    // fault
    if (!vmm.init(false))
      return 1;

    if (share_dir) {
      if (!vmm.start_virtiofsd(share_dir))
        return 1;
    }
    ...
    if (rc == 1 && snap_path) {
      ...
      if (!vmm.save_snapshot_file(snap_path))
        return 1;
  ...
  if (strcmp(cmd, "restore") == 0) {
    ...
    size_t snap_ram_mb = 64;
    {
      ...
      if (sfd >= 0) {
        SnapshotHeader hdr;
        if (::read(sfd, &hdr, sizeof(hdr)) == sizeof(hdr) &&
    ...
    if (ct_str)
      vmm.set_copy_threads(atoi(ct_str));
    if (has_flag(argc, argv, "--hugetlb"))
      vmm.set_hugetlb(true);
    ...
    if (cfg.has_net && cfg.tap[0]) {
      if (!vmm.connect_tap(cfg.tap))
        return 1;
    }
    long us_setup = us_since(t0);

    if (!vmm.restore_snapshot_file(snap_path))
      return 1;
    long us_snap = us_since(t0) - us_setup;

    if (vmm.timing_entrypoint())
      printf("[+%6.2fms] ", vmm.ms_since_start());
    printf("[VMM] restored in %.1fms  kvm_init=%.1fms virtiofsd=%.1fms "
           "tap=%.1fms snap=%.1fms (uffd=%d midway=%d)\n",
           us_since(t0) / 1000.0, us_init / 1000.0, us_virtiofsd / 1000.0,
           (us_setup - us_init - us_virtiofsd) / 1000.0, us_snap / 1000.0,
           vmm.use_uffd() ? 1 : 0, vmm.midway_uffd() ? 1 : 0);
  ...
  if (strcmp(cmd, "clone") == 0) {
    ...
    Snapshot golden;
    {
      ...
      ssize_t nr;
      nr = ::read(fd, golden.xsave, golden.hdr.xsave_size);
      (void)nr;
      ...
      if (golden.hdr.bitmap_bytes > 0) {
        ...
        nr = ::read(fd, golden.dirty_bitmap.data(), golden.hdr.bitmap_bytes);
        (void)nr;
      }
      ...
      while (rd < golden.ram_size) {
        ssize_t n =
            ::read(fd, (uint8_t *)golden.ram + rd, golden.ram_size - rd);
        if (n <= 0)
          break;
        rd += n;
      }
      ...
      if (pid == 0) {
        ...
        if (cfg.has_net) {
          ...
          if (share_dir) {
            ...
            if (sscanf(cfg.ip, "%u.%u.%u.%u/%u", &a, &b, &c, &d, &prefix) ==
                5) {
              snprintf(clone_cfg.ip, sizeof(clone_cfg.ip), "%u.%u.%u.%u/%u", a,
                       b, c, d + i, prefix);
            }
        ...
        if (!vmm.restore_snapshot(golden))
          _exit(1);
